...
class QLIGG_Setting_Controller extends QLIGG_Controller {
  ...
  function add_js() {
    if (isset($_GET['page']) && ($_GET['page'] === self::$slug)) {
      ...
      $settings = include_once(QLIGG_PLUGIN_DIR. 'assets/backend/js/settings.asset.php');
      wp_enqueue_script('qligg-admin-settings', plugins_url('/assets/backend/js/settings.js', QLIGG_PLUGIN_FILE), $settings['dependencies'], $settings['version'], true);
